...
class WidgetCollectionBase {
    ...
    getWidgetGeomObjBaseClass() {
        return WidgetGeomObjectBase;
    }
...
class WidgetCollection extends WidgetCollectionBase {
    ...
    _registerWidget(widget) {
        assert(widget instanceof this.getWidgetBaseClass() || widget instanceof this.getWidgetGeomObjBaseClass());
        assert(widget instanceof WidgetBase || widget instanceof WidgetGeomObjectBase);
    ...
    replaceItem(widgetOrIndex, newWidget) {
        ...
        const index = this._coerceToIndex(widgetOrIndex);
        assert(oldWidget instanceof WidgetBase || oldWidget instanceof WidgetGeomObjectBase);
    ...
    extractSubset(itemsToExtract) {
        ...
        function visit(item) {
            if (!is_visited(item)) {
                ...
                item[item.connectorsToVisitFn()]().forEach(connector => {
        ...
        element_to_create.forEach(item => {
            ...
            clone[item.connectorsToVisitFn()]().forEach(connector => {
