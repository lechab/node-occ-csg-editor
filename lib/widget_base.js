...
class WidgetBase extends WidgetBaseBase {
    ...
    constructor(name/*:string*/) {
        ...
        this.name = !name ? null : name; // replace undefined by null
        this._dependencies = {};
    ...
    _removeDependantEntity(entity) {
        ...
        if (!_shape) {
            // Object.keys(this._dependencies).forEach(x => this._dependencies[x] = null);
    ...
    getWidgetConnectors(optionalBaseClass) {
        ...
        return super.getWidgetConnectors(optionalBaseClass);
    ...
    dispose() {
        const filtered = _.uniq(this.getWidgetConnectors());
        filtered.forEach(wc => wc.dispose());
        this._id = "disposed";
    }
    ...
    connectorsToVisitFn() {
        return "getWidgetConnectors";
    }
}

class WidgetGeomObjectBase extends WidgetBaseBase {
    ...
    connectorsToVisitFn() {
        return "getGeometriesWidgetConnectors";
    }
    ...
    getGeometriesWidgetConnectors() {
        let result = [];
        for (let i = 0; i < this.geometries.length; i++) {
            result = result.concat(this.geometries[i].getWidgetConnectors());
        }
        return result;
    }
...
exports.WidgetGeomObjectBase = WidgetGeomObjectBase;
