...
function mergeStr(original, left, right) {
    ...
    const originalArray = splitRetain(original, "csg.fuse(");
    const leftArray = splitRetain(left, "csg.fuse(");
    const rightArray = splitRetain(right, "csg.fuse(");
...
function redefineNestedFuse(str, itNumber, nameOfShape, shapeNewDefinition) {
    ...
    if (!shapeNewDefinition) {
        return;
    }

    assert(shapeNewDefinition.name.indexOf(nameOfShape) > -1, "expecting the name of the newShape " +
        "to be included in the new Shape instance!");

    let strToInsert = shapeNewDefinition.toScript();
    strToInsert = "(" + strToInsert.substr(0, strToInsert.length - 1) + ")";

    const nbOfOccurences = str.match(new RegExp(nameOfShape, "g")).length;

    return str.replace(RegExp("^(?:.*?" + nameOfShape + "){" + parseInt(nbOfOccurences - itNumber) + "}"), function (x) {
...
class GeomPrimitiveObject extends GeomPrimitive {
    ...
    clone() {
        ...
        clone.filletFactor = this.filletFactor;
        clone.filletMode = this.filletMode;
        return clone;
    }
}
    ...
    toScript() {
        ...
        if (this.batchItems.length == 0 && it == 1) {
            ...
            if (this.isThereSomeRedefinitions) {
                ...
                const redefinitionIts = this.redefinedObjects.map(w => w.redefinitionIt).filter(u=>!!u);
    ...
    toScript() {
        ...
        if (this.batchItems.length == 0 && it == 1) {
            ...
            if (this.isThereSomeRedefinitions) {
                ...
                const redefinitionIts = this.redefinedObjects.map(w => w.redefinitionIt);
                const newDefinitions = this.redefinedObjects.map(w => w.redefinedGeomInstance);
                const namesOfRedefinedShapes = this.redefinedObjects.map(w => w.namesOfRedefinedShape);
