...
class GeometryEditor extends WidgetCollection {
    ...
    clone() {
        ...
        // clone.items.forEach(x => {
        //     x.parameters = x.parameters?clone.parametersEditor.items:null;
        // });

        clone.items.forEach(i => {
            if (!!i.geometries) {
                const found = this.items.filter(w => w.$$hashKey == i.$$hashKey)[0];
                i.parameters = !!found ? found.geometries[0].parameters : i.geometries[0].parameters;
            }
        });
        ...
        items.forEach(item => {
            ...
            if (item instanceof GeomPrimitiveObject) {
                item.parameters.forEach(p => {
                    p.id = p.id + "_" + item.getName();
                    p.name = p.name + "_" + item.getName();
                    p.displayName = p.displayName + "_" + item.getName();
                });
            }
