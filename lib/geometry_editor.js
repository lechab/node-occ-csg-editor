...
const WidgetGeomObjectBase = require("./widget_base").WidgetGeomObjectBase;
...
let future_GeomBase, future_GeomObjecBase;
...
class ShapeConnector extends WidgetConnector {
    ...
    isValidConnectedObject(object) {
        ...
        return object instanceof future_GeomBase || object instanceof future_GeomObjecBase;
    }
}
...
class GeomBase extends WidgetBase {
    ...
    cloneRedefinedGeom(clone) {
        // si la géométrie est issue d'une redéfinition
        // Alors on la clone
        if (!!this.geometry) {
    ...
    innerTransformationsString(isNested = false) {
        ...
        const isThereAnyTranslation = (
            (this.innerTranslation.vector.X.exp !== "0" && this.innerTranslation.vector.X.exp !== 0) ||
            (this.innerTranslation.vector.Y.exp !== "0" && this.innerTranslation.vector.Y.exp !== 0) ||
            (this.innerTranslation.vector.Z.exp !== "0" && this.innerTranslation.vector.Z.exp !== 0)
        );

        const isThereAnyRotation = this.innerRotation.angle.exp !== "0" && this.innerRotation.angle.exp !== 0;
    ...
    toFullScript(isNested = true) {
        return this.toScript(isNested);
    }

}

class GeomObjectBase extends WidgetGeomObjectBase {
...
future_GeomObjecBase = GeomObjectBase;

class GeomPrimitive extends GeomBase {

    evaluateParameters(parameters) {

        const e1 = evaluateExpressionPoint(this.innerTranslation.vector, parameters);
        this.innerTranslation.vector.set(e1[0], e1[1], e1[2]);

        const e2 = evaluateExpressionPoint(this.innerRotation.center, parameters);
        this.innerRotation.center.set(e2[0], e2[1], e2[2]);

        const e3 = evaluateExpressionPoint(this.innerRotation.axis, parameters);
        this.innerRotation.axis.set(e3[0], e3[1], e3[2]);

        const e4 = evaluateExpression(this.innerRotation.angle, parameters);
        this.innerRotation.angle.set(e4);
...
class GeomPrimitiveObject extends GeomObjectBase {
    ...
    evaluateParameters(parameters) {
        ...
        this.geometries.forEach(geom => geom.evaluateParameters(parameters));
    }

    getName() {
        // this.geometries[0].name  =>  means that every geometry share the geomObj. name
    ...
    toScript(isNested) {
        ...
        for (let j = 1; j < this.geometries.length; j++) {
...
class GeomPrimitiveBox extends GeomPrimitive {
    ...
    evaluateParameters(parameters) {
        super.evaluateParameters(parameters);

        const e1 = evaluateExpressionPoint(this.point1, parameters);
        this.point1.set(e1[0], e1[1], e1[2]);

        const e2 = evaluateExpressionPoint(this.point2, parameters);
        this.point2.set(e2[0], e2[1], e2[2]);
    }
    ...
    toVoluPrimitive(isNested = true) {
        return "csg.makeVoluBox("
            + "\"" + this.name + "\"" + ","
            + this.point1.toScript() + ","
            + this.point2.toScript() + ")"
            + this.innerTransformationsString(isNested);
...
class GeomPrimitiveCylinder extends GeomPrimitive {
    ...
    evaluateParameters(parameters) {
        ...
        this.radius.set(evaluateExpression(this.radius, parameters));
    }
...
class GeomPrimitiveCone extends GeomPrimitive {
    ...
    evaluateParameters(parameters) {
        ...
        this.radius1.set(evaluateExpression(this.radius1, parameters));
        this.radius2.set(evaluateExpression(this.radius2, parameters));
    }
...
class GeomPrimitiveSphere extends GeomPrimitive {
    ...
    evaluateParameters(parameters) {
        super.evaluateParameters(parameters);
        const e1 = evaluateExpressionPoint(this.center, parameters);
        this.center.set(e1[0], e1[1], e1[2]);
        this.radius.set(evaluateExpression(this.radius, parameters));
    }
...
class GeomPrimitiveTorus extends GeomPrimitive {
    ...
    constructor(name) {
        ...
        this.mainRadius.set(5);
        this.smallRadius = new Expression();
        this.smallRadius.set(1);
    }
    ...
    evaluateParameters(parameters) {
        ...
        const e2 = evaluateExpressionPoint(this.axis, parameters);
        this.axis.set(e2[0], e2[1], e2[2]);

        this.mainRadius.set(evaluateExpression(this.mainRadius, parameters));
        this.smallRadius.set(evaluateExpression(this.smallRadius, parameters));
    }
...
class GeomOperation extends GeomBase {
    ...
    evaluateParameters(parameters) {
        this.leftArg.get().evaluateParameters(parameters);
        this.rightArg.get().evaluateParameters(parameters);
    }

    toVoluPrimitive(isNested = true) {
        const operationName = this.constructor.name.split("GeomOperation")[1];
        return "await csg.makeVoluOperation("
            + "\"" + this.name + "\"" + ","
            + "\"" + operationName + "\"" + ","
            + "'" + serialijse.serialize(this.leftArg.get()) + "'" + ","
            + "'" + serialijse.serialize(this.rightArg.get()) + "'" + ")";
    }
...
class GeomOperationCut extends GeomOperation {
    ...
    toFullScript(isNested) {
        return "csg.cut(" + this.leftArg.get().toFullScript(true) + "," + this.rightArg.get().toFullScript(true) + ")"
            + this.innerTransformationsString();
    }
...
class GeomOperationCutPrimitive extends GeomBase {
    ...
    evaluateParameters(parameters) {
        this.leftArg.get().evaluateParameters(parameters);
        this.rightArg.get().evaluateParameters(parameters);
        this.thickness.set(evaluateExpression(this.thickness, parameters));
    }
...
class GeomOperationFuse extends GeomOperation {
    ...
    toFullScript(isNested) {
        return "csg.fuse(" + this.leftArg.get().toFullScript(true) + "," + this.rightArg.get().toFullScript(true) + ")"
            + this.innerTransformationsString();
    }
...
class GeomOperationCommon extends GeomOperation {
    ...
    toFullScript(isNested) {
        return "csg.common(" + this.leftArg.get().toFullScript(true) + "," + this.rightArg.get().toFullScript(true) + ")"
            + this.innerTransformationsString();
    }
...
class GeomTransfo extends GeomBase {
    ...
    evaluateParameters(parameters) {
        this.geometry.get().evaluateParameters(parameters);
    }
    ...
    toVoluPrimitive(isNested = true) {
        ...
        const transfoType = this.constructor.name.split("GeomTransfo")[1];
        const transfoDef = !!this.vector ? this.vector.toScript() :
            [this.center.toScript() + "," +
            this.axis.toScript() + "," +
            this.angle.toScript()];
        return "await csg.makeVoluTransfo("
            + "\"" + this.name + "\"" + ","
            + "\"" + transfoType + "\"" + ","
            + transfoDef + ","
            + "'" + serialijse.serialize(this.geometry.get()) + "'" + ")";
    }
...
class GeomTransfoTranslate extends GeomTransfo {
    ...
    // Batch transforms not taken into account here => TODO
    toFullScript(isNested) {
        return "(" + this.geometry.get().toFullScript(true) +
            ").translate(" +
    ...
    clone() {
        ...
        this.redefinedObjects = this.redefinedObjects || [];
...
class GeomTransfoRotate extends GeomTransfo {
    ...
    toFullScript(isNested) {
        return "((" + this.geometry.get().toFullScript(true) + ").rotate(" +
            this.center.toScript() + "," +
            this.axis.toScript() + "," +
            this.angle.toScript() + ");"
    }
...
class GeometryEditor extends WidgetCollection {
    ...
    _registerShape(shape) {
        assert(shape instanceof GeomBase || shape instanceof GeomObjectBase);
    ...
    // tree display (group objects depending on dependencies) helpers
